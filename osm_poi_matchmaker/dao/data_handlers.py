...
def count(model):
    with session_scope() as session:
        cnt = session.query(model.id).count()
    return cnt
...
def get_or_create_poi(model, **kwargs):
...
def search_poi_patch(model, **kwargs):
...
def get_or_create_common(model, **kwargs):
...
def insert_city_dataframe(city_df):
    db_count = count(City)
    ...
    try:
        for index, city_data in city_df.iterrows():
            get_or_create(City, city_post_code=city_data['city_post_code'],
...
def insert_street_type_dataframe(street_df):
    db_count = count(Street_type)
...
def insert_patch_data_dataframe(patch_df):
    db_count = count(POI_patch)
    ...
    try:
        for index, patch_data in patch_df.iterrows():
            ...
            get_or_create(POI_patch, poi_code=patch_data['poi_code'], orig_postcode=patch_data['orig_postcode'], orig_city=patch_data['orig_city'],
...
def insert_country_data_dataframe(country_df):
    db_count = count(Country)
    ...
    try:
        for index, country_data in country_df.iterrows():
            get_or_create(Country, country_code=country_data['country_code'], continent_code=country_data['continent_code'], country_name=country_data['country_name'],
...
def search_for_postcode(city_name):
    with session_scope() as session:
...
def insert_poi_dataframe(poi_df, raw=True):
    if raw is True:
    ...
    df_count = len(poi_df)
    if raw is True:
        db_count = count(POI_address_raw)
    else:
        db_count = count(POI_address)
    ...
    try:
        for poi_data in poi_dict:
            ...
            if raw is True:
                get_or_create_poi(POI_address_raw, **poi_data)
            else:
                get_or_create_poi(POI_address, **poi_data)
    except Exception as e:
...
def insert_type(type_data):
    try:
        for i in type_data:
            get_or_create_common(POI_common, **i)
...
def insert(**kwargs):
    try:
        ...
        get_or_create_poi(POI_address, **kwargs)
    except Exception as e:
