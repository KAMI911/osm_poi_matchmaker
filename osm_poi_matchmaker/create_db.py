...
def main():
    ...
    work = hu_tesco(db.session, 'http://tesco.hu/aruhazak/nyitvatartas/', DOWNLOAD_CACHE)
    ...
    work = hu_cba(db.session, 'http://www.cba.hu/uzletlista/', DOWNLOAD_CACHE)
    insert_type(db.session, work.types())
    work.process()

    logging.info('Importing {} stores ...'.format('Spar'))
    from osm_poi_matchmaker.dataproviders.hu_spar import hu_spar
    work = hu_spar(db.session, 'https://www.spar.hu/bin/aspiag/storefinder/stores?country=HU', DOWNLOAD_CACHE)
    insert_type(db.session, work.types())
    work.process()

    logging.info('Importing {} stores ...'.format('Rossmann'))
    from osm_poi_matchmaker.dataproviders.hu_rossmann import hu_rossmann
    work = hu_rossmann(db.session, 'https://www.rossmann.hu/uzletkereso', DOWNLOAD_CACHE)
    insert_type(db.session, work.types())
    work.process()

    logging.info('Importing {} stores ...'.format('KH Bank'))
    from osm_poi_matchmaker.dataproviders.hu_kh_bank import hu_kh_bank
    work = hu_kh_bank(db.session, os.path.join(DOWNLOAD_CACHE, 'hu_kh_bank.json'), 'K&H bank')
    insert_type(db.session, work.types())
    work.process()
    work = hu_kh_bank(db.session, os.path.join(DOWNLOAD_CACHE, 'hu_kh_atm.json'), 'K&H')
    work.process()
    ...
    work = hu_posta(db.session,
                    ...
                    DOWNLOAD_CACHE, 'hu_postacsekkautomata.json')
    work.process()
    ...
    work = hu_posta(db.session,
                    ...
                    DOWNLOAD_CACHE, 'hu_postaautomata.json')
    work.process()
    ...
    work = hu_posta(db.session,
                    ...
                    DOWNLOAD_CACHE, 'hu_postapoint.json')
    work.process()
    ...
    work = hu_penny_market(db.session, '', DOWNLOAD_CACHE, 'penny_market.json')
    insert_type(db.session, work.types())
    ...
    work = hu_foxpost(db.session,
                      'http://www.foxpost.hu/wp-content/themes/foxpost/googleapijson.php', DOWNLOAD_CACHE,
    ...
    work = hu_avia(db.session, 'https://www.avia.hu/kapcsolat/toltoallomasok/', DOWNLOAD_CACHE, 'hu_avia.html')
    insert_type(db.session, work.types())
    work.process()

    logging.info('Importing {} stores ...'.format('CIB Bank'))
    from osm_poi_matchmaker.dataproviders.hu_cib_bank import hu_cib_bank
    work = hu_cib_bank(db.session, '', os.path.join(DOWNLOAD_CACHE, 'hu_cib_bank.html'), 'CIB bank')
    insert_type(db.session, work.types())
    work = hu_cib_bank(db.session, '', os.path.join(DOWNLOAD_CACHE, 'hu_cib_atm.html'), 'CIB')
